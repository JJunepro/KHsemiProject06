...
//		private int bref;   // 진짜 원본글 조상글
	...
	public ArrayList<Board> selectBoardList(Connection conn, int start, int end) {
		...
		String sql = "select * from "
				...
				+ " (select * from board where bre_step=1 order by bref desc, bre_step asc) t1 ) t2 "
				+ " where r between ? and ?";
...
//			}
	...
	public int insertBoard(Connection conn, Board vo) {
		...
		// 답글, 답답....
		String sqlUpdate = "UPDATE board set bre_step=bre_step+1  where bref=? and bre_step>?";
		String sqlInsert = "INSERT INTO" + " board "
				+ " (BNO, TITLE, CONTENT, CREATE_DATE, WRITER, bref, bre_level, bre_step) "
				+ " VALUES (?, ?, ?, sysdate , ?, ?, ?, ?)";
